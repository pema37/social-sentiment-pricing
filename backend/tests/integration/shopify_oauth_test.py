...
def test_oauth_initiation():
    ...
    try:
        ...
        log_test(
            "OAuth init endpoint exists (POST /integrations/oauth/init)",
            r.status_code in [200, 401, 403, 422],
            f"Status: {r.status_code}" + (" (properly requires auth)" if r.status_code == 401 else ""),
        )
    except Exception as e:
    ...
    try:
        ...
        if r.status_code in [301, 302, 307, 308]:
            ...
            log_test("Frontend redirects on shop param", True, f"Redirects to: {location[:150]}")
    except Exception as e:
...
def test_oauth_callback():
    ...
    try:
        ...
        log_test(
            "Callback endpoint exists", r.status_code != 404, f"Status: {r.status_code}", critical=r.status_code == 404
        )
    except Exception as e:
    ...
    try:
        ...
        log_test(
            "Rejects invalid state (redirects to error)",
            r.status_code in [302, 400, 401, 403],
            f"Status: {r.status_code}" + (f" → {r.headers.get('location', '')[:100]}" if r.status_code == 302 else ""),
        )
    except Exception as e:
    ...
    for path, name in gdpr_endpoints:
        try:
            ...
            log_test(
                f"GDPR: {name}",
                is_found,
                f"Status: {r.status_code}" + (" (HMAC validation working)" if r.status_code == 401 else ""),
...
def test_frontend_app_bridge():
    ...
    try:
        ...
        log_test(
            "App Bridge script tag in HTML",
            has_app_bridge,
            "Found Shopify App Bridge CDN script"
            if has_app_bridge
            else 'MISSING: <script src="https://cdn.shopify.com/shopifycloud/app-bridge.js"></script>',
        ...
        log_test(
            ...
            has_session,
            "Found session token references"
            if has_session
            else "Not in initial HTML (may be in JS bundles — check manually)",
        )
...
def test_redirect():
    ...
    try:
        r = httpx.get(
            ...
            params={
                "shop": TEST_SHOP,
                "host": base64.b64encode(b"admin.shopify.com/store/demostore").decode(),
                "embedded": "1",
            },
            follow_redirects=False,
            timeout=15,
    ...
    for method, path, desc in checks:
        try:
            ...
            log_test(
                f"{desc} ({path})",
                ok,
                f"Status: {r.status_code}" + (" (properly requires auth)" if r.status_code in [401, 403] else ""),
            )
        except Exception as e:
