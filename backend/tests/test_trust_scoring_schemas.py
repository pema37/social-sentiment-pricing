...
class TestAuthorScoreRequest:
    ...
    def test_negative_follower_count_raises(self):
        with pytest.raises(ValidationError):
            AuthorScoreRequest(
                author_id="u1", username="t", source="x",
                follower_count=-1,
            )
...
class TestComponentScores:
    ...
    def test_below_zero_raises(self):
        with pytest.raises(ValidationError):
            ComponentScores(
                account_age=-0.1,
                followers=0, engagement=0, history=0, verification_bonus=0,
            )
    ...
    def test_boundary_values(self):
        c = ComponentScores(
            account_age=0.0, followers=1.0, engagement=0.0,
            history=1.0, verification_bonus=0.0,
        )
    ...
    def valid_data(self):
        return {
            ...
            "component_scores": ComponentScores(
                account_age=0.85, followers=0.65, engagement=0.70,
                history=0.50, verification_bonus=0.0,
            ),
            "confidence": 0.75,
            "calculated_at": NOW,
        }
    ...
    def valid_data(self):
        return {
            ...
            "spam_indicators": SpamIndicators(
                excessive_hashtags=False, excessive_links=False,
                keyword_stuffing=False, all_caps=False, spam_phrases=False,
            ),
            "is_spam": False,
        }
...
class TestBatchContentAnalysis:
    ...
    def test_batch_response(self):
        r = BatchContentAnalysisResponse(
            analyses=[], total=0, spam_count=0, duplicate_count=0,
        )
...
class TestMentionInput:
    ...
    def test_sentiment_score_above_range(self):
        with pytest.raises(ValidationError):
            MentionInput(
                mention_id="m", author_id="a", content="x",
                published_at=NOW, sentiment_score=1.5,
            )
...
def _mentions(n):
    ...
    return [
        MentionInput(
            mention_id=f"m{i}", author_id=f"a{i}",
            content=f"Content {i}", published_at=NOW,
        )
        for i in range(n)
    ]
...
class TestCampaignDetectionRequest:
    ...
    def test_time_window_min(self):
        with pytest.raises(ValidationError):
            CampaignDetectionRequest(
                mentions=_mentions(5), time_window_hours=0,
            )

    def test_time_window_max(self):
        with pytest.raises(ValidationError):
            CampaignDetectionRequest(
                mentions=_mentions(5), time_window_hours=169,
            )
...
class TestCampaignSignalResponse:
    ...
    def test_strength_above_one_raises(self):
        with pytest.raises(ValidationError):
            CampaignSignalResponse(
                signal_type="test", strength=1.5, description="x",
            )
    ...
    def test_valid_minimal(self):
        r = WeightedSentimentRequest(
            mentions=[MentionInput(
                mention_id="m1", author_id="a1",
                content="Good stuff", published_at=NOW,
            )]
        )
...
class TestQualityMetrics:
    ...
    def test_valid(self):
        q = QualityMetrics(
            high_trust_ratio=0.35, filtered_count=15, confidence=0.72,
        )
...
class TestQuickSpamCheck:
    ...
    def test_response_valid(self):
        r = QuickSpamCheckResponse(
            is_spam=False, spam_score=0.1, reasons=[],
        )
        assert r.is_spam is False

    def test_response_spam_detected(self):
        r = QuickSpamCheckResponse(
            is_spam=True, spam_score=0.95,
...
class TestQuickTrustCheck:
    ...
    def test_request_valid(self):
        r = QuickTrustCheckRequest(
            author_id="u1", username="test", source="twitter",
        )
        assert r.follower_count is None
    ...
    def test_response_score_range(self):
        with pytest.raises(ValidationError):
            QuickTrustCheckResponse(
                is_trustworthy=True, trust_score=-0.1,
                trust_level=TrustLevelEnum.HIGH, risk_flags=[],
            )
