        ...
        integration_summary.append(
            {
                ...
                if hasattr(integration, "last_sync_at") and integration.last_sync_at
                else None,
            }
        )
    ...
    for product in products:
        ...
        for link in product_links:
            ...
            if integration:
                ...
                int_status = (
                    integration.status.value if hasattr(integration.status, "value") else str(integration.status)
                )
                ...
                platforms_linked.append(
                    {
                        ...
                        "external_price": live_external_price
                        if live_external_price is not None
                        else (float(link.external_price) if link.external_price is not None else None),
...
async def check_product_push_status(
    ...
    all_links_stmt = select(ProductIntegrationLink).where(ProductIntegrationLink.product_id == product_id)
    ...
    if all_links:
        int_stmt = select(Integration).where(Integration.id.in_([l.integration_id for l in all_links]))
        int_result = await db.execute(int_stmt)
    ...
    for link in all_links:
        ...
        will_push = link.sync_enabled and integration and integration.status == IntegrationStatus.ACTIVE
...
def _resolve_variant_price(external_product: Any, external_variant_id: str | None) -> float | None:
