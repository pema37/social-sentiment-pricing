...
from core.rate_limit import limiter, ANALYSIS_RATE_LIMIT
...
async def diagnose_product_recommendations(
    ...
    if not competitor_products:
        diagnosis["issues"].append(
            "NO_COMPETITOR_PRODUCTS: No competitor products linked to this product"
        )
    ...
    if rules and not active_rules:
        diagnosis["issues"].append(
            "NO_ACTIVE_RULES: Rules exist but none are active"
        )
