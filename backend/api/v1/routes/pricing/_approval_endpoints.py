...
async def approve_recommendation(
    ...
    try:
        ...
        recommendation = await service.auto_approve_and_apply(
            recommendation_id, 
            current_user.id
        )
        logger.info(
            f"Recommendation {recommendation_id} approved and applied "
        ...
        logger.warning(
            f"Approval failed for {recommendation_id}: [{e.error_code}] {e.message}"
        )
...
async def reject_recommendation(
    ...
    try:
        recommendation = await service.reject(
            recommendation_id, 
            current_user.id, 
            data.reason
        )
...
async def apply_recommendation(
    ...
    try:
        ...
        logger.info(
            f"Recommendation {recommendation_id} applied by user {current_user.id}"
        )
        return recommendation
