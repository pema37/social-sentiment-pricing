...
def generate_mock_signals(competitor: str, has_launch: bool = True) -> list[LaunchSignal]:
    ...
    if has_launch:
        return [
            LaunchSignal(source="twitter", content=f"{competitor} just announced their new product line! Exciting features coming soon. #newlaunch"),
            LaunchSignal(source="reddit", content=f"Anyone else see the {competitor} announcement? Looks like they're releasing a competitor to our favorite product."),
            LaunchSignal(source="news", content=f"{competitor} unveils next-generation product with AI features, targeting premium market segment."),
        ]
    return [
...
async def stream_launch_analysis(
    ...
    image: Optional[UploadFile] = File(None, description="Product screenshot")
):
    """
