...
class SyncProductRequest(BaseModel):
    ...
    integration_id: UUID | None = Field(
        None, description="Specific integration to sync to. If None, syncs to all active integrations."
    )
...
class LinkProductRequest(BaseModel):
    ...
    external_variant_id: str | None = Field(None, description="Variant ID (for Shopify)")
...
class BulkSyncRequest(BaseModel):
    ...
    product_ids: list[UUID] | None = Field(
        None, description="Specific product IDs to sync. If None, syncs all products without links."
    )
    integration_id: UUID | None = Field(
        None, description="Specific integration to sync to. If None, syncs to all active integrations."
    )
...
async def sync_product_to_store(
    product_id: UUID,
    request: SyncProductRequest | None = None,
    ...
    if request and request.integration_id:
        ...
        if not integration:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Integration not found or not active")
    ...
    sync_result = await sync_service.sync_product_on_create(product=product, user_id=current_user.id, auto_push=True)

    return {"product_id": str(product_id), "product_name": product.name, **sync_result}
...
async def link_product_to_store(
    ...
    stmt = select(Integration).where(Integration.id == request.integration_id, Integration.user_id == current_user.id)
    result = await db.execute(stmt)
    integration = result.scalars().first()
    ...
    if not link_result["success"]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=link_result.get("error", "Failed to create link")
        )
...
async def get_product_sync_status(
    ...
    stmt = select(Integration).where(
        Integration.user_id == current_user.id, Integration.status == IntegrationStatus.ACTIVE
    )
    result = await db.execute(stmt)
...
async def bulk_sync_products(
    request: BulkSyncRequest | None = None,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    """
...
async def unlink_product_from_store(
    ...
    stmt = select(ProductIntegrationLink).where(
        ProductIntegrationLink.product_id == product_id, ProductIntegrationLink.integration_id == integration_id
    )
    result = await db.execute(stmt)
