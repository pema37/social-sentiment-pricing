...
async def generate_prospect_audit(request: ProspectAuditRequest, req: Request):
    ...
    if not request.store_url and not request.products:
        raise HTTPException(
            status_code=422,
            detail="Provide either a store_url or a list of products."
        )
...
async def generate_prospect_pdf(request: ProspectPDFRequest, req: Request):
    ...
    if not products:
        raise HTTPException(
            status_code=422,
            detail="Could not fetch products from the provided store URL."
        )
    ...
    for r in all_results:
        ...
        lost_rev = Decimal(str(abs(gap_pct) * 0.015 * 3 * float(r.your_price) * 30)).quantize(Decimal("0.01")) if r.gap_type == "overpriced" else Decimal("0")
        missed_margin = Decimal(str(abs(float(r.your_price) - float(r.market_avg_price or 0)) * 3 * 30)).quantize(Decimal("0.01")) if r.gap_type == "underpriced" and r.market_avg_price else Decimal("0")

        sku_results.append(SKUAuditResult(
