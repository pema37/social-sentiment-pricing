...
from models.integration import EcommercePlatform, Integration

from ..base import EcommerceService
from ..repositories import LinkRepository, ProductRepository
...
class ProductSyncHandler:
    ...
    async def _create_or_link(
        ...
        variant_price: float | None = None,
        variant_compare_at_price: float | None = None,
    ) -> tuple[int, int]:
        ...
        if sibling_link:
            logger.info(
                f"Product for {external_product.id} exists (via sibling variant), adding variant link {variant_id}"
            )
