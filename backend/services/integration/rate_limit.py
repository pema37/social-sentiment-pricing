...
class RateLimitTracker:
    ...
    async def update_from_response(self, store_url: str, headers: dict, platform: str = "shopify"):
    ...
    async def mark_rate_limited(self, store_url: str, retry_after: int | None = None):
    ...
    async def wait_if_needed(self, store_url: str) -> float:
        ...
        if state.should_wait():
            ...
            if wait_time > 0:
                logger.info(f"Rate limit active for {store_url}, waiting {wait_time:.2f}s")
