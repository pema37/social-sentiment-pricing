...
from .retry import DEFAULT_RETRY_CONFIG, RetryConfig, execute_with_retry
...
class RetryableClient:
    ...
    async def _request(self, method: str, url: str, operation_name: str = "request", **kwargs) -> httpx.Response:
        """Make request with retry, rate limit, and circuit breaker"""

        if self._circuit_breaker:
            async with self._circuit_breaker:
                return await self._do_request_with_retry(method, url, operation_name, **kwargs)
        else:
            return await self._do_request_with_retry(method, url, operation_name, **kwargs)

    async def _do_request_with_retry(self, method: str, url: str, operation_name: str, **kwargs) -> httpx.Response:
        ...
        async def _do_request():
            ...
            await rate_limit_tracker.update_from_response(self.store_url, dict(response.headers), self.platform)

            if response.status_code == 429:
                retry_after = response.headers.get("Retry-After")
                await rate_limit_tracker.mark_rate_limited(self.store_url, int(retry_after) if retry_after else None)

            response.raise_for_status()
            return response

        return await execute_with_retry(_do_request, config=self.retry_config, operation_name=operation_name)
