...
class AnalyticsService:
    ...
    async def get_dashboard_overview(self) -> DashboardOverview:
        ...
        # Product counts
        result = await self.session.execute(select(func.count(Product.id)).where(Product.user_id == self.user_id))
        ...
        # Competitor count
        result = await self.session.execute(select(func.count(Competitor.id)).where(Competitor.user_id == self.user_id))
        total_competitors = result.scalar_one()

        # Alert counts
        result = await self.session.execute(
            select(func.count(Alert.id)).where(and_(Alert.user_id == self.user_id, Alert.status == AlertStatus.SENT))
        )
        unread_alerts = result.scalar_one()

        result = await self.session.execute(
            select(func.count(Alert.id)).where(and_(Alert.user_id == self.user_id, Alert.created_at >= today_start))
        )
        ...
        result = await self.session.execute(
            ...
            .where(and_(Product.user_id == self.user_id, Sentiment.analyzed_at >= now - timedelta(hours=24)))
        )
    ...
    async def get_product_summaries(self, limit: int = 10) -> list[ProductSummary]:
        ...
        result = await self.session.execute(
            select(Product).where(Product.user_id == self.user_id).order_by(Product.updated_at.desc()).limit(limit)
        )
    ...
    async def get_recommendation_stats(self, days: int = 30) -> RecommendationStats:
        ...
        result = await self.session.execute(
            select(func.count(PriceRecommendation.id)).where(
                and_(PriceRecommendation.user_id == self.user_id, PriceRecommendation.created_at >= since)
            )
        )
    ...
    async def get_alert_analytics(self, days: int = 7) -> AlertAnalytics:
        ...
        result = await self.session.execute(
            select(Alert).where(and_(Alert.user_id == self.user_id, Alert.created_at >= since))
        )
    ...
    async def get_sentiment_trend(
        self, product_id: str | None = None, days: int = 30, bucket: str = "day"
    ) -> SentimentAnalytics:
        ...
        query = (
            ...
            .join(Product, Sentiment.product_id == Product.id)
            .where(and_(Product.user_id == self.user_id, Sentiment.analyzed_at >= start))
        )
    ...
    async def _get_average_sentiment(self, hours: int = 24, offset_hours: int = 0) -> float | None:
        ...
        result = await self.session.execute(
            ...
            .join(Product, Sentiment.product_id == Product.id)
            .where(and_(Product.user_id == self.user_id, Sentiment.analyzed_at >= start, Sentiment.analyzed_at < end))
        )
