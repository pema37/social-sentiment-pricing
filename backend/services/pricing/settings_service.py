...
class SettingsService:
    ...
    async def get_settings(self, user_id: UUID) -> PricingSettings | None:
        """
        ...
        """
        logger.warning(f"get_settings() called for user {user_id} - consider using get_or_create() instead")
        stmt = select(PricingSettings).where(PricingSettings.user_id == user_id)
        result = await self.db.execute(stmt)
    ...
    async def _create_default_settings(self, user_id: UUID) -> PricingSettings:
        ...
        settings = PricingSettings(user_id=user_id, **DEFAULT_SETTINGS)
    ...
    async def update_settings(self, user_id: UUID, **kwargs) -> PricingSettings:
        """
    ...
    def check_requires_approval(
        self, product: Product, change_percent: Decimal, confidence: Decimal, settings: PricingSettings | None
    ) -> bool:
        """
        ...
        if confidence < settings.auto_approve_min_confidence:
            logger.debug(
                f"Requires approval: confidence {confidence} < threshold {settings.auto_approve_min_confidence}"
            )
            return True
        ...
        if change_percent > 0 and change_percent > settings.auto_approve_max_increase:
            logger.debug(
                f"Requires approval: increase {change_percent}% > threshold {settings.auto_approve_max_increase}%"
            )
            return True
        ...
        if change_percent < 0 and abs(change_percent) > settings.auto_approve_max_decrease:
            logger.debug(
                f"Requires approval: decrease {abs(change_percent)}% > threshold {settings.auto_approve_max_decrease}%"
            )
            return True
