...
class SignalProcessor:
    ...
    async def _get_sentiment_signals(self, product_id: UUID) -> tuple[Decimal | None, Decimal | None]:
        ...
        current_stmt = select(func.avg(Sentiment.compound_score)).where(
            Sentiment.product_id == product_id, Sentiment.analyzed_at >= last_24h
        )
        ...
        prev_stmt = select(func.avg(Sentiment.compound_score)).where(
            Sentiment.product_id == product_id, Sentiment.analyzed_at >= last_48h, Sentiment.analyzed_at < last_24h
        )
    ...
    async def _get_volume_signals(self, product_id: UUID) -> tuple[int, int]:
        ...
        count_stmt = select(func.count(SocialMention.id)).where(
            SocialMention.product_id == product_id, SocialMention.published_at >= last_24h
        )
    ...
    async def _get_viral_signals(self, product_id: UUID) -> tuple[bool, int, int, Decimal | None]:
        ...
        stmt = (
            ...
            .where(SocialMention.product_id == product_id, SocialMention.published_at >= last_24h)
            .order_by(SocialMention.engagement_count.desc())
            .limit(10)
        )
    ...
    async def _get_trend_signals(self, product_id: UUID) -> dict:
        ...
        trend_direction, trend_strength = self._determine_trend(mention_growth_rate, sentiment_momentum)
    ...
    async def _get_daily_sentiment(self, product_id: UUID, days: int = 7) -> list[Decimal | None]:
        ...
        for i in range(days):
            ...
            stmt = select(func.avg(Sentiment.compound_score)).where(
                Sentiment.product_id == product_id, Sentiment.analyzed_at >= day_start, Sentiment.analyzed_at < day_end
            )
            result = await self.db.execute(stmt)
            avg = result.scalar()
    ...
    def _calculate_momentum(self, daily_values: list[Decimal | None]) -> Decimal:
        """
    ...
    def _determine_trend(self, growth_rate: Decimal, sentiment_momentum: Decimal) -> tuple[str, Decimal]:
        """
