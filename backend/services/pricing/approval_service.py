...
class ApprovalService:
    ...
    async def approve(self, recommendation_id: UUID, user_id: UUID, auto: bool = False) -> PriceRecommendation:
        """
        ...
        if recommendation.status != RecommendationStatus.PENDING:
            raise ApprovalError(f"Cannot approve recommendation with status: {recommendation.status}", "INVALID_STATUS")

        if recommendation.valid_until < datetime.now(UTC):
            recommendation.status = RecommendationStatus.EXPIRED
            self.db.add(recommendation)
            await self.db.commit()
            raise ApprovalError("This recommendation has expired. Please generate a new one.", "RECOMMENDATION_EXPIRED")

        recommendation.status = RecommendationStatus.AUTO_APPROVED if auto else RecommendationStatus.APPROVED
    ...
    async def reject(self, recommendation_id: UUID, user_id: UUID, reason: str | None = None) -> PriceRecommendation:
        """Reject a recommendation."""
        recommendation = await self._get_recommendation(recommendation_id, user_id)

        if recommendation.status != RecommendationStatus.PENDING:
            raise ApprovalError(f"Cannot reject recommendation with status: {recommendation.status}", "INVALID_STATUS")
    ...
    async def apply_price(self, recommendation_id: UUID, user_id: UUID) -> PriceRecommendation:
        """
        ...
        if recommendation.status not in [RecommendationStatus.APPROVED, RecommendationStatus.AUTO_APPROVED]:
            raise ApprovalError(f"Cannot apply recommendation with status: {recommendation.status}", "INVALID_STATUS")
    ...
    async def auto_approve_and_apply(
        ...
        if recommendation.status != RecommendationStatus.PENDING:
            raise ApprovalError(
                f"Cannot auto-apply recommendation with status: {recommendation.status}", "INVALID_STATUS"
            )
        ...
        if not platform_result.get("success"):
            ...
            error_msg = platform_result.get("error", "Unknown error pushing to platform")
            error_code = platform_result.get("error_code", "PLATFORM_PUSH_FAILED")
            logger.error(f"Auto-apply push failed for recommendation {recommendation_id}: [{error_code}] {error_msg}")
            raise ApprovalError(error_msg, error_code)
    ...
    async def process_auto_approvals(self, user_id: UUID) -> list[PriceRecommendation]:
        ...
        for rec in pending:
            ...
            if change > 0 and change > float(settings.auto_approve_max_increase):
                logger.debug(f"Skipping {rec.id}: increase {change}% > max {settings.auto_approve_max_increase}%")
                continue
            if change < 0 and abs(change) > float(settings.auto_approve_max_decrease):
                logger.debug(f"Skipping {rec.id}: decrease {abs(change)}% > max {settings.auto_approve_max_decrease}%")
                continue
    ...
    async def _get_user_settings(self, user_id: UUID) -> PricingSettings | None:
    ...
    async def get_approval_stats(self, user_id: UUID, days: int = 30) -> dict:
        ...
        avg_confidence_applied: float | None = float(avg_confidence) if avg_confidence else None
    ...
    async def _record_decision(
        ...
        try:
            ...
            logger.warning(f"Failed to record outcome for recommendation {recommendation_id}: {e}")
