...
from .mnee_client import MneeClient, MneeEnvironment
from .exceptions import MneeValidationError, MneeConfigError
    ...
    def require_valid_address(self, address: str, field_name: str = "address") -> str:
        ...
        if address.startswith("0x"):
            raise MneeValidationError(
                f"Ethereum addresses (0x...) are not supported. "
                f"MNEE uses BSV addresses starting with '1' or '3'.",
                field=field_name,
            )
        
        if not self.validate_bsv_address(address):
            raise MneeValidationError(
                f"Invalid BSV address. Must start with '1' or '3' "
                f"and be 25-34 characters.",
                field=field_name,
            )
...
_service_instance: Optional[MneeService] = None
