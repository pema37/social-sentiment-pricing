...
class TrendCalculators:
    ...
    def detect_basic_trends(sentiment_data: list[dict]) -> list[dict]:
        ...
        if len(sentiment_data) >= 7:
            ...
            diff = recent_avg - older_avg
            if diff > 0.2:
                trends.append({"type": "sentiment_rise", "description": f"Sentiment increased by {diff:.2f} points"})
            elif diff < -0.2:
                trends.append(
                    {"type": "sentiment_drop", "description": f"Sentiment decreased by {abs(diff):.2f} points"}
                )
