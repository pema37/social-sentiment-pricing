...
class CrisisDetector:
    ...
    async def run_monitor_agent(
        self, sentiment_data: list[SentimentDataPoint], product_name: str, baseline_sentiment: float = 0.0
    ) -> AsyncGenerator[CrisisAgentMessage]:
        """
...
Flag anything where sentiment dropped more than 30% or volume spiked more than 2x."""
        ...
        anomaly_detected, metrics = self._calculate_anomaly_metrics(sentiment_data, baseline_sentiment)
    ...
    async def run_investigator_agent(
        self, sentiment_data: list[SentimentDataPoint], product_name: str, monitoring_result: dict
    ) -> AsyncGenerator[CrisisAgentMessage]:
        """
...
- Scale of impact (isolated incident vs. widespread)"""
    ...
    async def run_response_agent(
        self, product_name: str, monitoring_result: dict, investigation_result: dict
    ) -> AsyncGenerator[CrisisAgentMessage]:
        """
...
```"""
        ...
        yield CrisisAgentMessage(
            ...
            is_final=True,
            metadata={"response_plan": response_data, "full_analysis": full_response},
        )
    ...
    def _prepare_data_summary(self, data: list[SentimentDataPoint], baseline: float) -> str:
    ...
    def _calculate_anomaly_metrics(self, data: list[SentimentDataPoint], baseline: float) -> tuple[bool, dict]:
        """Calculate anomaly metrics from sentiment data."""
        if not data:
            return False, {"sentiment_change": 0, "volume_change": 0, "current_sentiment": baseline}
        ...
        # Detect anomaly
        anomaly_detected = (
            sentiment_change < self.sentiment_drop_threshold or volume_change > self.volume_spike_threshold
        )
    ...
    def _get_negative_samples(self, data: list[SentimentDataPoint], limit: int = 20) -> str:
        ...
        for d in negative[:limit]:
            samples.append(f'[{d.source}] Score: {d.score:.2f}\n"{d.sample_text}"')
    ...
    def _get_affected_sources(self, data: list[SentimentDataPoint]) -> list[str]:
        ...
        return [source for source, scores in source_scores.items() if sum(scores) / len(scores) < 0]
