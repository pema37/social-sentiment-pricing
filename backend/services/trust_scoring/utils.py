...
def is_bot_username(username: str) -> bool:
    ...
    return any(re.match(pattern, username_lower) for pattern in BOT_USERNAME_PATTERNS)


def calculate_spam_score(text: str, username: str | None = None) -> float:
    """
...
def calculate_account_age_score(
    account_age_days: int | None, new_threshold: int = 30, established_threshold: int = 180
) -> float:
    """
...
def calculate_follower_score(follower_count: int | None, following_count: int | None = None) -> float:
    """
...
def detect_burst_activity(timestamps: list[datetime], window_minutes: int = 60, burst_threshold: int = 10) -> bool:
    """
