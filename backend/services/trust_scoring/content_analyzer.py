...
class ContentAnalyzer:
    ...
    def analyze(
        ...
        content_quality_score = self._calculate_quality_score(
            text, metrics, risk_flags, author_username
        )
        
        originality_score = self._calculate_originality_score(
            is_duplicate, duplicate_count, similarity_to_recent
        )
    ...
    def analyze_batch(
        ...
        """
        return [
            self.analyze(content_id, text, username)
            for content_id, text, username in contents
        ]
