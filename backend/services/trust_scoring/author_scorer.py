...
class AuthorScorer:
    ...
    def score_author(
        ...
        history_score, history_flags = self._calculate_history_score(profile, historical_posts)
    ...
    def _calculate_history_score(
        ...
        timestamps = [p.get("published_at") for p in historical_posts if p.get("published_at")]
        ...
        sentiments = [p.get("sentiment_score") for p in historical_posts if p.get("sentiment_score") is not None]
    ...
    def get_cached_score(self, author_id: str) -> AuthorTrustScore | None:
...
_default_scorer: AuthorScorer | None = None
