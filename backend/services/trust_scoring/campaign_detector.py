...
class CampaignDetector:
    ...
    def detect(
        ...
        timing_score = max((s.strength for s in signals if "timing" in s.signal_type.lower()), default=0.0)
        similarity_score = max((s.strength for s in signals if "similarity" in s.signal_type.lower()), default=0.0)
    ...
    def _analyze_timing_patterns(
        self,
        mentions: list[MentionData],
    ) -> tuple[CampaignSignal | None, set[str]]:
        """
    ...
    def _detect_burst_activity(
        ...
        if not detect_burst_activity(
            timestamps, window_minutes=self.burst_window_minutes, burst_threshold=self.burst_threshold
        ):
            return None, suspicious_authors
...
_default_detector: CampaignDetector | None = None
    ...
    for m in mentions:
        mention_data.append(
            MentionData(
                ...
                published_at=m.get("published_at", datetime.now(UTC)),
