...
class TrendDetector:
    ...
    async def detect_volume_spike(
        self, product_id: UUID, current_hours: int = 6, baseline_hours: int = 48
    ) -> dict | None:
        """
    ...
    async def detect_sentiment_shift(
        self, product_id: UUID, current_hours: int = 12, previous_hours: int = 12
    ) -> dict | None:
        """
        ...
        velocity_data = await self.aggregator.get_sentiment_velocity(
            product_id, current_hours=current_hours, previous_hours=previous_hours
        )
    ...
    async def detect_viral_mentions(self, product_id: UUID, hours: int = 24) -> dict | None:
        """
        ...
        viral_mentions = [m for m in mentions if (m.engagement_count or 0) >= self.viral_engagement_threshold]
        ...
        if viral_mentions:
            ...
            overall = (
                "positive"
                if positive_viral > negative_viral
                else "negative"
                if negative_viral > positive_viral
                else "mixed"
            )
