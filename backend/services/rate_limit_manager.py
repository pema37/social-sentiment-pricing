...
class CircuitBreaker:
    ...
    def record_failure(self, error_message: str = "", is_rate_limit: bool = False):
        ...
        if is_rate_limit or self.failure_count >= self.failure_threshold:
            ...
            logger.warning(f"Circuit {self.name} → OPEN for {self.cooldown_seconds}s. Reason: {error_message}")
...
class RateLimitManager:
    ...
    def record_rate_limit(self, api_name: str, retry_after: int | None = None):
...
def record_api_rate_limit(api_name: str, retry_after: int | None = None):
