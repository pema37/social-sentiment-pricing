...
async def broadcast_price_update(product_id: str, data: dict):
    ...
    await manager.broadcast("prices", {"type": "price_update", "product_id": product_id, "data": data})


async def broadcast_alert(alert_data: dict):
    """Call from alert service when new alerts are created."""
    await manager.broadcast("alerts", {"type": "new_alert", "data": alert_data})
...
async def broadcast_sentiment_update(product_id: str, sentiment_data: dict):
    ...
    await manager.broadcast_sentiment(
        product_id, {"type": "sentiment_update", "product_id": product_id, "data": sentiment_data}
    )
...
async def broadcast_mention_received(product_id: str, mention_data: dict):
    ...
    await manager.broadcast_sentiment(
        product_id, {"type": "new_mention", "product_id": product_id, "data": mention_data}
    )
