    ...
    <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/5 p-6">
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        ...
        <div className="rounded-lg bg-muted/50 p-4 text-center">
          ...
          <p className={`text-3xl font-bold ${isIncrease ? "text-emerald-600" : isDecrease ? "text-red-600" : "text-muted-foreground"}`}>
            {isIncrease ? "+" : ""}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {recommendation.key_factors.map((factor) => (
              <span key={factor} className="rounded-full bg-muted border px-3 py-1 text-xs text-muted-foreground">
                {factor}
              </span>
            ))}
          </div>
