...
const CorrelationGraph: React.FC<CorrelationGraphProps> = ({ title, dataPoints1, dataPoints2, labels = [] }) => {
    
    // Safety checks for graph calculation
    const maxVal1 = dataPoints1.length > 0 ? Math.max(...dataPoints1) : 0;
    const minVal1 = dataPoints1.length > 0 ? Math.min(...dataPoints1) : 0;
    const range1 = maxVal1 - minVal1 || 1; // Prevent division by zero

    const maxVal2 = dataPoints2.length > 0 ? Math.max(...dataPoints2) : 0;
    const minVal2 = dataPoints2.length > 0 ? Math.min(...dataPoints2) : 0;
    const range2 = maxVal2 - minVal2 || 1; // Prevent division by zero
                ...
                <div className="absolute left-8 right-8 bottom-0 top-0">
                    ...
                    <svg className="w-full h-full absolute" viewBox="0 0 100 100" preserveAspectRatio="none">
                        ...
                        <polyline
                            ...
                            points={dataPoints1.map((val: number, i: number) =>
                                // The calculation is strictly typed as numbers
                        ...
                        <polyline
                            ...
                            strokeLinecap="round"
                            strokeLinejoin="round"
                            points={dataPoints2.map((val: number, i: number) =>
                                // The calculation is strictly typed as numbers
...
export default function ProductDetailPage({ params }: ProductDetailPageProps): JSX.Element {
    ...
    // In a real application, you would fetch this data using the productId
    // const productData = useFetchProduct(productId); 
    
    // --- Placeholder Product Data (Typed as ProductDetailData) ---
        ...
        <div className="bg-gray-100 min-h-screen">
            <main className="flex-1 p-4 sm:p-6 md:p-8 max-w-7xl mx-auto"> 
                ...
                <div className="flex justify-between items-center mb-6 pt-4">
                    <Link href="/products" className="text-blue-600 hover:text-blue-800 flex items-center transition-colors font-medium">
                        <span className="mr-2 text-xl">←</span> Go back to Products
                    </Link>
                    <Link href={`/products/edit/${productData.id}`} passHref legacyBehavior>
                        <a className="bg-blue-500 hover:bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg shadow transition-colors duration-200">
                            Edit Product
                        </a>
                    </Link>
                </div>
                ...
                <div className="mb-8 border-b pb-4">
                    ...
                    {/* Note: The backlink was fixed to /products assuming that is the correct route */}
